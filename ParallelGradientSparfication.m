% Here is the distributed parallel gradient method with 
% gradient sparsification where 
% we select a set of K coordinates randomly 

function [Fcn, Coordinates_sent] = ParallelGradientSparfication(A,b,Iter,beta,delta,num_group,K)
...
for ii = 1:num_group  
    ...
    a(:,ii) = RandomizedCoordinate( a(:,ii) , K ); % g_t^p
...
if mod(k,1000) == 0 
fprintf('Parallel Gradient Sparsification with num_worker = %d: Function = %d    Coordinates sent = %d     Time = %d \n',num_group,Fcn(k),K,time);    
end
